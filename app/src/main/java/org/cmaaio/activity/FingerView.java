...
public class FingerView extends View {
	...
	private Path mPath;
	private Paint mBitmapPaint;// 画布的画�?
	private Paint mPaint;// 真实的画�?
	private float mX, mY;// 临时点坐�?
	private static final float TOUCH_TOLERANCE = 4;

	// 保存Path路径的集�?用List集合来模拟栈
	...
	private void init(int w, int h) {
		...
		mPaint.setStrokeJoin(Paint.Join.ROUND);// 设置外边�?
		mPaint.setStrokeCap(Paint.Cap.SQUARE);// 形状
		mPaint.setStrokeWidth(7);// 画笔宽度
	...
	public void onDraw(Canvas canvas) {
		// canvas.drawColor(color);//背景颜色，不设置为�?明，否则看不到后面的自定义edittext�?
		// 将前面已经画过得显示出来
	...
	private void touch_move(float x, float y) {
		...
		if (dx >= TOUCH_TOLERANCE || dy >= TOUCH_TOLERANCE) {
			// 从x1,y1到x2,y2画一条贝塞尔曲线，更平滑(直接用mPath.lineTo也是可以�?
	...
	/**
	 * 撤销的核心�?想就是将画布清空�?将保存下来的Path路径�?���?��移除掉， 重新将路径画在画布上面�?
	 */
	public void undo() {
		mBitmap = Bitmap.createBitmap(screenWidth, screenHeight,
				Bitmap.Config.ARGB_8888);
	...
	/**
	 * 重做的核心�?想就是将撤销的路径保存到另外�?��集合里面(�?�?然后从redo的集合里面取出最顶端对象�?画在画布上面即可�?
	 */
	public void redo() {
		// 如果撤销你懂了的话，那就试试重做吧�?
	}
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			/*System.out.println("滑动时坐�?----====X坐标:" + x + "----====Y坐标:" + y);*/
			if (null != fingerMatrix) {
				fingerMatrix.setX(x);
				fingerMatrix.setY(y);
			}
	...
	Handler handler = new Handler() {
		public void handleMessage(Message msg) {
			...
			case 1:
				/*System.out
						.println("*********0.handler发消息告诉activity处理数据 *********");*/
				...
				if (null != fingerMatrix) {
					...
					System.out.println("矩阵的坐标信息为�?======maxX:" + maxX
							+ "----====maxY:" + maxY + "----====minX:" + minX
							+ "----====minY:" + minY);
					...
					// tempBitmap = Bitmap.createBitmap(tempBitmap, (int)minX,
				...
				// //这里目前传�?的是全屏的bitmap，应该为绘制�?��x,y轴到�?��xy轴的区域 +�?��空白区域的图�?
